import torch
import itertools
from gans.util.image_pool import ImagePool
...
class CycleGANModel(BaseModel):
    ...
    def __init__(self, config):
        ...
        self.net_G_A = networks.define_G(config['input_nc'], config['output_nc'], config['ngf'], config['netG'], config['norm'],
                                        not config['no_dropout'], config['init_type'], config['init_gain'], self.gpu_ids)
        self.net_G_B = networks.define_G(config['output_nc'], config['input_nc'], config['ngf'], config['netG'], config['norm'],
                                        not config['no_dropout'], config['init_type'], config['init_gain'], self.gpu_ids)

        if self.isTrain:  # define discriminators
            self.net_D_A = networks.define_D(config['output_nc'], config['ndf'], config['netD'],
                                            config['n_layers_D'], config['norm'], config['init_type'], config['init_gain'], self.gpu_ids)
            self.net_D_B = networks.define_D(config['input_nc'], config['ndf'], config['netD'],
                                            config['n_layers_D'], config['norm'], config['init_type'], config['init_gain'], self.gpu_ids)

        if self.isTrain:
            if config['lambda_identity'] > 0.0:  # only works when input and output images have the same number of channels
            ...
            self.optimizer_G = torch.optim.Adam(itertools.chain(self.net_G_A.parameters(), self.net_G_B.parameters()), lr=config['lr'], betas=(config['beta1'], 0.999))
            self.optimizer_D = torch.optim.Adam(itertools.chain(self.net_D_A.parameters(), self.net_D_B.parameters()), lr=config['lr'], betas=(config['beta1'], 0.999))
