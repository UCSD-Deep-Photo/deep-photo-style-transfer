...
class BaseDataset(data.Dataset, ABC):
    ...
    -- <__init__>:                      initialize the class, first call BaseDataset.__init__(self, config).
...
def get_transform(config, params=None, grayscale=False, method=Image.BICUBIC, convert=True):
    transform_list = []
    if grayscale:
...
def __print_size_warning(ow, oh, w, h):
    ...
    if not hasattr(__print_size_warning, 'has_printed'):
        logging.warning("The image size needs to be a multiple of 4. "
