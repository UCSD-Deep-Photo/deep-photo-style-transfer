...
from munit.utils import get_config, get_data_loader_folder, pytorch03_to_pytorch04, load_inception
...
logging.basicConfig(filename='out/munit_test_{}.log'.format(config['timestamp']), 
                    level=logging.INFO, 
...
if opts.trainer == 'MUNIT':
    ...
    if opts.compute_IS:
        logging.info("Inception Score: {}".format(np.exp(np.mean(IS))))
    if opts.compute_CIS:
        logging.info("conditional Inception Score: {}".format(np.exp(np.mean(CIS))))
